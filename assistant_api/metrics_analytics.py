...
page_views = Counter(
    "page_view_total",
    "Page views",
    ["path", "ref_host", "device", "theme", "region", "ua_is_bot"],
)
...
scroll_depth = Counter(
    "scroll_depth_percent_total", "Scroll depth events", ["path", "percent"]
)
...
project_plays = Counter(
    "project_video_play_total", "Project video plays", ["project_id"]
)
agent_requests = Counter(
    "agent_request_total", "Agent requests", ["intent", "project_id"]
)
agent_feedback = Counter(
    "agent_feedback_total", "Agent feedback", ["sentiment", "intent"]
)
...
click_bins = Counter(
    "click_bin_total", "Binned click coordinates", ["x_bin", "y_bin", "path"]
)
...
link_clicks = Counter(
    ...
    [
        "kind",
        "href_domain",
    ],  # kind ∈ {github, artstation, resume}; href_domain is hostname
)
