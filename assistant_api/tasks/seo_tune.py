...
def heuristic_rewrite(meta: PageMeta, url: str, target_ctr: float) -> PageMeta:
    ...
    if len(desc) < 80:
        desc = (desc + " Fast load, clear value, and real outcomes. Learn how this project improves workflow and reliability.").strip()
...
def run(threshold: float|None = None) -> Dict:
    ...
    web_root = settings["WEB_ROOT"] or "."  # point to built HTML (e.g., apps/web/dist or repo root)
    ...
    json_path = write_artifact(settings["ARTIFACTS_DIR"], "seo-tune.json", json.dumps(out_json, ensure_ascii=False, indent=2))

    # minimal MD
    lines = ["# SEO Tune Report", "", f"- Generated: {out_json['generated']}", f"- Threshold: {th}", f"- Pages: {len(pages)}", ""]
    for p in pages[:200]:
        lines += [
