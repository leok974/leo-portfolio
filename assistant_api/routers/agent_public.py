...
ALLOWED_LOGO_HOSTS = [
    h.strip() for h in os.getenv("SITEAGENT_LOGO_HOSTS", "").split(",") if h.strip()
]
...
@router.post("/run")
async def run_agent(
    request: Request, task: str | None = Query(None), body: bytes = Depends(_authorized)
):
...
def report():
    ...
    status_data = _read(
        "siteAgent.json", {"ts": None, "brand": "LEO KLEMET — SITEAGENT"}
    )
    # artifacts (optional)
    artifacts = {}
...
async def act(req: Request, body: bytes = Depends(_authorized)):
    ...
    if payload.task:
        ...
        if task_name == "pr.create":
            ...
            body_text = task_payload.get(
                "body",
                "Auto-generated by SiteAgent.\n\nIncludes: layout sections/preset.",
            )
            ...
            if not result.get("ok"):
                raise HTTPException(
                    500, detail=f"PR creation failed: {result.get('error')}"
                )
...
async def automation_workflow(
    include: str | None = Query(
        None, description="Comma-separated list of tasks to include"
    ),
    exclude: str | None = Query(
        None, description="Comma-separated list of tasks to exclude"
    ),
    ...
    inc_set = set([t.strip() for t in (include or "").split(",") if t.strip()]) or set(
        task_all
    )
    ...
    run_lines = []
    banner = (
        "echo '⚠️  DRY-RUN ENABLED - No changes will be committed'" if dry_run else ""
    )
    ...
    for t in tasks:
        if t == "media.optimize":
            ...
            cmd = (
                f"python -m assistant_api.cli run {t} {'--dry-run' if dry_run else ''}"
            )
...
@router.post("/run_now")
async def run_now(
    req: Request, body: bytes = Depends(_authorized), preset: str | None = None
):
    """
...
async def autotune(
    req: Request, body: bytes = Depends(_authorized), alpha: float = 0.5
):
    """
