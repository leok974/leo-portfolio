...
class AgentTaskCreate(BaseModel):
    ...
    task: str = Field(
        ..., max_length=64, description="Task name (e.g., 'seo.validate')"
    )
    run_id: str = Field(
        ..., max_length=64, description="Run identifier (e.g., 'nightly-2025-01-15')"
    )
    status: str = Field(
        ...,
        max_length=32,
        description="Initial status (typically 'queued' or 'running')",
    )
    started_at: datetime | None = Field(None, description="Task start timestamp")
    inputs: dict[str, Any] | None = Field(
        None, description="Task-specific inputs (flags, config)"
    )
...
class AgentTaskUpdate(BaseModel):
    ...
    outputs_uri: str | None = Field(
        None, max_length=512, description="Link to PR, artifact, or report"
    )
    log_excerpt: str | None = Field(
        None, description="First/last N lines of stdout/stderr"
    )
    approval_state: str | None = Field(
        None, max_length=32, description="pending | approved | rejected | cancelled"
    )
    approver: str | None = Field(
        None, max_length=128, description="User who approved/rejected"
    )
    approval_note: str | None = Field(
        None, description="Approval/rejection/cancellation reason"
    )
    webhook_notified_at: datetime | None = Field(
        None, description="Webhook notification timestamp"
    )
