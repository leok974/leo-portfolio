...
test.describe('Portfolio Assistant panel', () => {
  ...
  test('Layout panel renders JSON when layout exists', async ({ page }) => {
    ...
    page.on('request', req => {
      if (req.url().includes('api') || req.url().includes('layout')) {
        console.log('[REQUEST]', req.method(), req.url());
      }
    });

    page.on('response', res => {
      if (res.url().includes('api') || res.url().includes('layout')) {
    ...
    await page.route('**/*', async (route) => {
      ...
      // Match /api/layout with or without query parameters
      if (url.includes('/api/layout')) {
        console.log('[TEST] Intercepting /api/layout request:', url);
        // Return a LayoutRecipe with version and cards (not grid/weights)
    ...
    // Wait for assistant panel to be visible first
    const panel = page.locator('[data-testid="assistant-panel"]');
    ...
    await layoutToggle.waitFor({ state: 'visible', timeout: 5000 });
    await layoutToggle.click();

    // Wait a moment for the layout section to expand
    await page.waitForTimeout(500);

    // The layout should have been fetched on page load and our mock should have returned it
    // Look for the JSON block directly
    const jsonBlock = page
      .locator('[data-testid="assistant-layout-json"]')
      .or(page.locator('pre'));

    // If not visible, try clicking refresh to trigger another fetch
    const refresh = page.locator('[data-testid="assistant-layout-refresh"]');
    if (await jsonBlock.isVisible() === false && await refresh.isVisible()) {
      console.log('[TEST] Clicking refresh to trigger fetch');
      await refresh.click();
    ...
    await expect(jsonBlock).toContainText('"version"');
